...
if __name__ == '__main__':
    ...
    # 定义模型
    model = TextCNN(vocab_size, embedding_size, feature_size, seq_length, window_sizes, num_classes, 0.5)
    criterion = nn.CrossEntropyLoss()
    opt = optim.Adam(model.parameters(), lr=0.001)

    if os.path.exists("./save/text_cnn.pth") and False:
        model = torch.load("./save/text_cnn.pth")
        ...
        # 保存模型
        # torch.save(model, './save/text_cnn.pth')
