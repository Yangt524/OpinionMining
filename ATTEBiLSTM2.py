...
def main(batch_size=50, embedding_size=300, n_hidden=300, num_classes=2, dr_rate=0.4, load=True,
         save=False, file_path='./save/atte2.pth', data_path='./data/om/', epochs=20, learning_rate=5e-5):
    ...
    # 定义模型
    model = OpinionMining3(vocab_size, embedding_size, n_hidden, num_classes, dr_rate=dr_rate)
    ...
    if os.path.exists(file_path) and load:
        ...
        for epoch in range(epochs):
            ...
            for train_batch_data, train_batch_target, train_batch_label in train_loader:
                pre = model(train_batch_data, train_batch_target)
            ...
            with torch.no_grad():
                for dev_batch_data, dev_batch_target, dev_batch_label in dev_loader:
                    pre = model(dev_batch_data, dev_batch_target)
    ...
    with torch.no_grad():
        for test_batch_data, test_batch_target, test_batch_label in test_loader:
            pre = model(test_batch_data, test_batch_target)
            # 负例预测分数
